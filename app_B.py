...
# 从环境变量获取账号密码和 TG 配置
EMAIL_B        = os.environ.get("LUNES_EMAIL_B") or ""     # 登录邮箱
PASSWORD_B     = os.environ.get("LUNES_PASSWORD_B") or ""  # 登录密码
...
def send_tg_message(status_icon, status_text, extra_text=""):
    ...
    if '@' in EMAIL_B:
        name, domain = EMAIL_B.split('@', 1)
        if len(name) > 4:
            masked_email = f"{name[:2]}****{name[-2:]}@{domain}"
        else:
            masked_email = f"{name}@{domain}"
    else:
        masked_email = EMAIL_B[:2] + '****'
...
def login(sb) -> bool:
    ...
    print(f"📧 填写邮箱...")
    js_fill_input(sb, 'input[name="email"]', EMAIL_B)
    time.sleep(0.3)
    
    print("🔑 填写密码...")
    js_fill_input(sb, 'input[name="password"]', PASSWORD_B)
    time.sleep(1)
